def main():
    ...
    formula_result = parse_formula(formula, periodic_table )

    molar_mass = compute_molar_mass(formula_result, periodic_table)
...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    for symbol, quantity in symbol_quantity_list:
        ...
        # symbol_key = periodic_table[symbol][ATOMIC_MASS_INDEX] #Get Atomic Mass
        symbol_key = periodic_table[symbol]
        atomic_mass = symbol_key[ATOMIC_MASS_INDEX]
        product_result = atomic_mass * quantity
        elements_sum += product_result
